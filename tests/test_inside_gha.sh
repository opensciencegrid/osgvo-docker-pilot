...
COMMON_DOCKER_ARGS="run --user osg
                        ...
                        -e _CONDOR_ALL_DEBUG='D_CAT,D_SUB_SECOND,D_PID,D_SECURITY:2'
...
function test_docker_HAS_SINGULARITY {
    ...
    has_singularity=$(run_inside_backfill_container env _CONDOR_SEC_CLIENT_AUTHENTICATION_METHODS=FS condor_status -startd -debug:D_SECURITY:2 -direct "$startd_addr" -af HAS_SINGULARITY); ret=$?
    [[ $ret -eq $ABORT_CODE ]] && { debug_docker_backfill; return $ABORT_CODE; }
